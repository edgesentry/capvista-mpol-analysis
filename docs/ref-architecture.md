...
Watchlist Parquet includes `ownership_chain` (JSON array of hops for the Ownership chain panel) when the ownership graph has edges for that MMSI. If the column is missing on R2, run indago `data-publish` (or `push-arktrace` after a local pipeline) — ensure indago uses the `score/` watchlist path, not stale flat copies at `data/processed/` root. UI behaviour and verification steps: [ref-ownership-chain-ui.md](ref-ownership-chain-ui.md).
